...
var dynamicDateFormat = timeFormat([
    [d3.time.format("%m-%d"), function() {  return true; }],
    [d3.time.format("%m-%d %H:%S"), function() { b=brush.extent(); extent=b[1]-b[0]; if(extent<10*ms_in_day) return true;  }],
    //[function(){return "";}, function(d) { return d.getDate() != 1; }]
]);
var current_scale="data";
...
if(dateRange < ms_in_day){
    var button_data =["hour","data"];
}
else if(dateRange < ms_in_month){
    var button_data =["day","hour","data"];
}

else if (dateRange < ms_in_year)   {
    var button_data =["month","day","hour","data"];
} 
else {
    var button_data =["year","month","day","hour","data"];
};
...
function customTickFunction(t0, t1, dt)  {
    ...
    //var b =brush.extent();
    var extent = t1.getTime()-t0.getTime();
    function step_days(date, offset)
    {
        date.setDate(date.getDate() + offset);
    }
    function step_hours(date, offset)
    {
        date.setHours(date.getHours() + offset);
    }
    function step_minutes(date, offset)
    {
    ...
    //console.info(time);
    if(extent>3*ms_in_day){


        var time = d3.time.day.ceil(t0);
       
        var times = [], dayFactors = [1,2,3,4,5,6,7];

        while (time < t1) times.push(new Date(time)), step_days(time, 1);
        var timesCopy = times;
        var i;
        ...
        for(i=0 ; times.length > maxTotalLabels ; i++)
            times = _.filter(timesCopy, function(d){
                return (d.getDate()) % dayFactors[i] == 0;
            });
        //console.info(times);
    }
    else if(extent<3*ms_in_day){
        var time = d3.time.hour.ceil(t0);
        //time.setMinutes(t0.getMinutes()-t0.getMinutes()%15);
        var times = [], hourFactors=[1,4];

        while (time < t1) times.push(new Date(time)), step_hours(time, 3);
        var timesCopy = times;
        var i;
        ...
        for(i=0 ; times.length > maxTotalLabels ; i++)
            times = _.filter(timesCopy, function(d){
                return (d.getHours()) % hourFactors[i] == 0;
            });
    }/*
    else if(extent>ms_in_hour){
         
        var time = t0;
        time.setMinutes(t0.getMinutes()-t0.getMinutes()%15);
        var times = [], minuteFactors=[1,30];

        while (time < t1) times.push(new Date(time)), step_minutes(time, 15);
        var timesCopy = times;
        var i;
        ...
        for(i=0 ; times.length > maxTotalLabels ; i++)
            times = _.filter(timesCopy, function(d){
                return (d.getMinutes()) % minuteFactors[i] == 0;
            });
    }*/
...
// action for buttons that scale focus to certain time interval
    ...
    else if (d == "day") {interval_ms = 86400000;}
    else if (d=="hour") {interval_ms = 3600000;}

    if ( d == "year" | d == "month" | d=="day" | d=="hour")  {
