...
def run_tool(entry_point,arg_values, arg_types,user):
    ...
    if ".py" in entry_point.split("/")[-1]:
        ...
        output = client.containers.run(
            image_name,
            command=command,#"matlab -nodisplay -nosplash -nodesktop -r \"exit\"",#"matlab -nodisplay -nosplash -nodesktop -r \"fid=fopen('/ypan12/ag_data/canopyheight.txt', 'w');fclose(fid);exit\"",
            # command=[main_cmd, script_path],
            volumes={f"/data/{user}": {"bind": f"/{user}", "mode": "rw"}},
            # working_dir=working_dir,
